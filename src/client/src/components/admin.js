...
class Admin extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .post(`http://${window.location.hostname}:8080/api/adminGetProfiles`, {
        userID,
      })
      .then((res) => {
        console.log(res);
  ...
  saveEdits(user) {
    ...
    const acceptFunction = () => {
      ...
      axios
        .post(`http://${window.location.hostname}:8080/api/adminUpdateProfile`, {
          userID,
  ...
  deleteUser(user, index) {
    ...
    this.openWarningDialog(
      ...
      () => {
        axios.post(
          `http://${window.location.hostname}:8080/api/adminDeleteProfile`,
